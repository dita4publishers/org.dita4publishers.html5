...
<xsl:template name="compas">
  ...
  <xsl:variable name="icon">
   ...
   <xsl:variable name="href">
    ...
    <div class="{concat($direction, 'link')}"><a href="{$href}" title="{linktext}"><span class="{$icon}"></span><span class="{$CLASSSCREENREADER}"><xsl:value-of select="linktext" /></span></a></div><xsl:value-of select="$newline"/>
</xsl:template>
